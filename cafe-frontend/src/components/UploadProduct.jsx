...
const UploadProduct = ({ onClose, fetchData }) => {
  ...
  // Calculate discount percentage - not needed for cafe
  ...
  const validateForm = useCallback(() => {
    ...
    if (!data.productName.trim()) {
      newErrors.productName = 'Menu item name is required';
    } else if (data.productName.trim().length < 3) {
      newErrors.productName = 'Menu item name must be at least 3 characters';
    }
        ...
        <div className='flex justify-between items-center p-6 border-b border-gray-200 bg-gradient-to-r from-amber-50 to-orange-50'>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-amber-600 rounded-full flex items-center justify-center">
              <BiCoffee className="text-white text-xl" />
            </div>
            <div>
              <h2 id="modal-title" className='text-xl font-bold text-gray-800'>Add Menu Item</h2>
              <p className="text-sm text-gray-600">Add a new item to your cafe menu</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-5">
              ...
              >
                <input 
                  type='text' 
                  placeholder='e.g., Cappuccino, Blueberry Muffin, Caesar Salad' 
                  name='productName'
